...
public class ContentDAO {
	...
	// Thêm mới hàm getCountContentByUserID
	public int getCountContentByUserID(int id) throws SQLException {
		int total = 0;
        // try-with-resource statement will auto close the connection.
        try (Connection connection = conn.getConnection(); 
        		PreparedStatement preparedStatement = connection.prepareStatement("select count(*) from content where author_id=?")) {
            System.out.println(preparedStatement);
	...
	// Thêm mới hàm getContentByAuthorID
	public ArrayList<Content> selecContentByAuthorID(int authorid)  {

		ArrayList<Content> contents = new ArrayList<Content>();
        // try-with-resource statement will auto close the connection.
        try (Connection connection = conn.getConnection(); 
        		PreparedStatement preparedStatement = connection.prepareStatement("select * from content where author_id = ?")) {
